$(function () {
    ...
    async function karatsuba(x, y, n = null, nivel = '0', parent = $etapasContainer) {
        ...
        const[xStr, yStr] = [x.toString(), y.toString()];
        const B = parseInt(xStr.slice(0, n - q)) || 0;
        const A = parseInt(xStr.slice(n - q)) || 0;
        console.log(yStr);
        const D = parseInt(yStr.slice(0, n - q)) || 0;
        const C = parseInt(yStr.slice(n - q)) || 0;

        highlightLine('linha3');
        const $ab = await adicionarEtapa($detalhes, `ab_${nivel}`, `A \\gets X[${q+1}..${x.toString().length}], \\quad B \\gets X[1..${q}]`);
        $ab.html(`\\(A \\gets ${A}, \\quad B \\gets ${B}\\)`);
        await MathJax.typesetPromise([$ab[0]]); await delay(400);

        highlightLine('linha4');
        const $cd = await adicionarEtapa($detalhes, `cd_${nivel}`, `C \\gets Y[${q+1}..${y.toString().length}], \\quad D \\gets Y[1..${q}]`);
        ...
        const $G = await adicionarEtapa($detalhes, `G_${nivel}`, `G \\gets \\text{Karatsuba}(${A + B}, ${C + D}, ${Math.ceil(n / 2) + 1})`, 500);
        const G = await karatsuba(A + B, C + D, Math.ceil(n / 2) + 1, nivel + '1', $detalhes);
        ...
        $H.html(`\\(H \\gets ${G} - E - F \\)`); await MathJax.typesetPromise([$H[0]]); await delay(400);
        $H.html(`\\(H \\gets ${G} - ${E} - F \\)`); await MathJax.typesetPromise([$H[0]]); await delay(400);
        $H.html(`\\(H \\gets ${G} - ${E} - ${F}\\) `); await MathJax.typesetPromise([$H[0]]); await delay(400);
        $H.html(`\\(H \\gets ${G - E} - ${F} \\)`); await MathJax.typesetPromise([$H[0]]); await delay(400);
